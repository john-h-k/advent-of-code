...
let parse_rounds_legal comps =
  String.split_on_chars comps ~on:[ ';' ]
  |> List.fold_left
       ~f:(fun acc round ->
         acc && round |> String.split_on_chars ~on:[ ',' ] |> parse_round_legal)
       ~init:true
;;

let parse_game_id id =
  let parts = String.split_on_chars id ~on:[ ' ' ] in
    ...
    (match String.split_on_chars head ~on:[ ' ' ] with
     | number :: [ color ] ->
       let new_max =
         ...
         |> int_of_string
         |> max
              (match Hashtbl.find maxs color with
               | Some x -> x
               | None -> 0)
       in
...
let parse_rounds_max comps =
  let maxs = Hashtbl.create (module String) in
  [ "red"; "green"; "blue" ]
  |> List.iter ~f:(fun v ->
    let _ = Hashtbl.add maxs ~key:v ~data:0 in
    ());
  let rounds = String.split_on_chars comps ~on:[ ';' ] in
  rounds
  |> List.iter ~f:(fun r -> parse_round_max maxs (String.split_on_chars r ~on:[ ',' ]));
  maxs
;;
