...
let get_day_input year day =
  let session = 
    ...
    | Some s -> s
    | None -> failwith "must set `AOC_SESSION` env var to your advent of code session cookie"
  in
  let uri = Printf.sprintf "https://adventofcode.com/%d/day/%d/input" year day |> Uri.of_string in
